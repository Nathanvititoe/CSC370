...
from src.dataset_setup.dataset_loader import build_dataset_list, get_loaders
...
def setup_dataset(DATASET_DIR, CLASS_MAP, CLASS_NAMES, img_size, batch_size): 
    ...
    print("--- Class counts BEFORE loader ---")
    print("Train:", Counter(label for _, label in train_ds))
    print("Val  :", Counter(label for _, label in val_ds))
    print("----------------------------------")

    # get dataset loaders
    train_loader, val_loader = get_loaders(train_ds, val_ds, img_size, batch_size)
